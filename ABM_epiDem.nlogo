...
[
  ...
  beta-n                ;; The average number of new secondary                                                     ; transmission rate
                        ;; infections per infected this tick
  gamma                 ;; The average number of new recoveries                                                    ; recovery rate
                        ;; per infected this tick
  r0                    ;; The number of secondary infections that arise                                           ; basic reproduction number
                        ;; due to a single infected introduced in a wholly
                        ;; susceptible population
                        ;; Each person decides to get vax'd or not once/day.
  lst-infected          ;; list of infection counts for each day
  peak-max                                                                                                         ; The maximum number of hourly infections throughout the simulation.
]
...
[
  ...
  vaccinated?                                                                                                      ; If true, the susceptible person has been vaccinated.
  vaccine_prob                                                                                                     ; Probability of getting vaccinated,
                                                                                                                   ; which increases when infections reach 80% of population.

  riskgroup2?                                                                                                      ; Risk group 2 has a higher infection chance than risk group 1.
  ...
  nb-vaccinated                                                                                                    ; Number of people vaccinated at the end of the tick
]
...
to setup-people
  ...
  [
    setxy random-xcor random-ycor ;;                                                              ; spreading out turtles across x and y coordinate plane
    set cured? false ;;                                                                           ; no one is cured in beginning
    set infected? false ;;                                                                        ; no one is infected in beginning
    set susceptible? true ;;                                                                      ; everyone is susceptible in beginning
    set vaccinated? false ;;                                                                      ; no one is vaccinated in beginning
    set vaccine_prob random-float 1 ;;                                                            ; assing random probability of getting vaccinated
    set riskgroup2? False
    ...
    [                                                                                             ; don't want recovery time to be too long
      set recovery-time average-recovery-time * 2
    ]
    if recovery-time < 0 [ set recovery-time 0 ]                                                  ; making sure it's positive. with norm dist, can get neg numbers

    ;; Each individual has a 5% chance of starting out infected.                                  ; some turtles start out infected
  ...
  ask turtles [
    create-links-with n-of 8 other turtles                                                        ; turtles create links with 8 other turtles
...
to go
  ...
  ask turtles [
    ...
    let n-pro-links (count my-in-links with [[vaccine_prob] of other-end > 0.5])                 ; count number of pro-vaccine links
    let n-anti-links (count my-in-links with [[vaccine_prob] of other-end < 0.5])                ; count number of anti-vaccine links
    ifelse n-pro-links > n-anti-links                                                            ; If pro-vaccine links > than anti-vaccine links,
      [set vaccine_prob vaccine_prob * local-pro-factor]                                         ; increase probability of getting vaccinated,
      [set vaccine_prob vaccine_prob * local-anti-factor]                                        ; else decrease probability of getting vaccinated.
  ]
  ...
  tick
  set lst-infected lput count turtles with [infected?] lst-infected
  set peak-max max lst-infected
end
...
to vaccine_prob_inc
  if (count turtles with [infected?] / (count turtles) > global-pro-factor-threshold)            ; if 50% of population is infected,
  [set vaccine_prob vaccine_prob * global-pro-factor]                                            ; increase probability of getting vaccinated by 1%
end

to vaccinate
  if susceptible? [                                                                              ; If a person is susceptible
    if vaccine_prob > 0.9                                                                        ; and if their probability of getting vaccinated is greater than XX%,
      [
        set vaccinated? true                                                                     ; move them to vaccinated state.
        set nb-vaccinated (nb-vaccinated + 1)
    ]                                                                                            ; Add this person to the number of vaccinated people.
  ]
end
...
to infect  ;; turtle procedure
   let nearby-uninfected (turtles-on neighbors)
     with [ not infected? and not cured? ]                                                       ; If not infected and not cured, then there is chance of infection.

     if nearby-uninfected != nobody
     [ ask nearby-uninfected
       [

      let temp_risk_prob 1
      let temp_infection-chance 1

      if vaccinated? [set temp_risk_prob risk_prob]                                              ; If vaccinated, lower infection chance.
      if riskgroup2? [set temp_infection-chance infection-chance2]

      if random-float 100 < infection-chance * temp_risk_prob * temp_infection-chance
         [ set infected? true
...
to calculate-r0
  ...
  set nb-infected-previous                                                              ; number of infected turtles in previous time step
    count turtles with [ infected? ] +
    new-recovered - new-infected

  ;; Number of susceptibles now:                                                        ; S(t) = N - I(t) - R(t)
  let susceptible-t
    initial-people -
    count turtles with [ infected? ] -
  ...
  [ set beta-n 0 ]                                                                      ; transmission rate
  [
    ;; This is beta-n, the average number of new                                        ; effective reproduction number
    ;; secondary infections per infected per tick
    set beta-n (new-infected / nb-infected-previous)
  ]

  ifelse nb-infected-previous < 10
  [ set gamma 0 ]                                                                       ; recovery rate
  [
  ...
  [
    ;; This is derived from integrating dI / dS = (beta*SI - gamma*I) / (-beta*SI):     ; differential equations
...
"Vaccinated" 1.0 0 -10141563 true "" "plot (count turtles with [vaccinated?])"
...
1209
409
Cumulative Compartments (% of total pop)
hours
% of pop
0.0
10.0
...
"% vaccinated" 1.0 0 -10141563 true "" "plot ((count turtles with [ vaccinated? ] / initial-people) * 100)"
...
263
% Vaccinated
(count turtles with [vaccinated?] / count turtles) * 100
2
1
11
...
759
91
if vaccinated, multiply the infection chance by this number. 
11
0.0
1
...
804
142
if most of my nearest neighbors are vaccinated, multiply probababilty of getting vaccinated by this factor
11
0.0
1
...
804
208
if most of my nearest neighbors are not vaccinated, multiply probability of getting vaccinated by this number
11
0.0
1
...
831
293
if infection prevalence is more than the global-pro-factor-threshold, multiply probability of getting vaccinated by this number
11
0.0
1
...
767
349
if the infection prevalence above this number, multiply the probability of getting vaccinated by the global-pro-factor
11
0.0
1
...
146
Peak Hour
position peak-max lst-infected
17
1
11
...
791
442
Risk group 2 has a higher infection chance than risk group 1 to simulate a sub-population that is more vulnerable to infection.
11
0.0
1
